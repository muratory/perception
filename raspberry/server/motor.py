...
# Mimic Differential Behavior for 2 Back/Left Motor
# Adjust the Power based on the steering angle value
SPEED_LIMIT_OFFSET = 5.
Kp = (0. - SPEED_LIMIT_OFFSET)/(50. - 0.)
gSpeed = 0
...
def setSpeedAdjust(angle):
        global gSpeed
        offset = int(Kp * angle)

        speedM0 = gSpeed + offset
        speed = gSpeed
        speedM1 = gSpeed - offset

        speedM0 *= 40
        speed *= 40
        speedM1 *= 40

        print ('speedM0:', speedM0, 'speed: ', speed, 'speedM1:', speedM1)
        p.set_value(EN_M0, 0, speedM0)
        p.set_value(EN_M1, 0, speedM1)
