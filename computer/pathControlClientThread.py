...
class pathControlCommandClientThread(commonThreadSocket):
    """ Implements the threading.Thread interface (start, join, etc.) and
        can be controlled via the cmd_q Queue attribute. Replies are placed in
        the reply_q Queue attribute.
    """
    def __init__(self):
        super(pathControlCommandClientThread, self).__init__()
            ...
            while True:
                ...
                try:
                    ...
                    if len(header_data) == 4:
                        ...
                        dataPathControlCommand = self._recv_n_bytes(msg_len)
                        if len(dataPathControlCommand) == msg_len:
                            self.reply_q.put(self._success_reply(dataPathControlCommand))
                    else:
                        #for whatever reason the len does not match
                        self.reply_q.put(self._error_reply('pathControlCommandClient socket misalignement'))
                        
                except IOError as e:
                    if type(e) == socket.timeout:
                        #this could be possible since we don't want to block the task forever
                        #but could be a real issue to get data in
                        print 'Warning : no data received for pathControlCommand client'
                        continue
                    else:
